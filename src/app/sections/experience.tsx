...
export default function ExperienceSection({ lng }: { lng: 'en' | 'no' }) {
  const [ready, setReady] = useState(false);

  useEffect(() => {
    let alive = true;
    setReady(false);
    getI18nClient(lng).finally(() => alive && setReady(true));
    return () => {
      alive = false;
    };
  }, [lng]);
...
function ExperienceSectionInner() {
  ...
  return (
    <section id="experience" className="mx-auto my-16 w-full max-w-3xl px-4">
      ...
      <div className="mx-auto mb-8 flex w-full max-w-md items-center justify-center gap-2 rounded-xl border border-zinc-200 bg-white/70 p-1 dark:border-zinc-800 dark:bg-zinc-950/50">
        <button
          onClick={() => setTab('work')}
...
function Pill({ children }: { children: React.ReactNode }) {
  return (
    <span className="rounded-full border border-zinc-300 px-2 py-0.5 text-xs text-zinc-700 dark:border-zinc-800/70 dark:text-zinc-300">
      {children}
    </span>
  );
}
...
function TimelineDot({ kind }: { kind: Kind }) {
  ...
  return (
    <div className="flex h-7 w-7 items-center justify-center rounded-full border border-zinc-300 bg-white text-[10px] font-semibold text-zinc-700 shadow dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-200">
      {label}
    </div>
  );
}
...
function Card({ item, showMoreLabel, showLessLabel }: CardProps) {
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white/70 p-4 shadow-sm backdrop-blur dark:border-zinc-800/70 dark:bg-zinc-900/60">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          {hasMore && (
            <button
              ...
              className="ml-auto text-xs underline underline-offset-2 transition text-zinc-600 hover:text-zinc-800 dark:text-zinc-400 dark:hover:text-zinc-200"
            >
