...
import enMessages from '../languages/en/experience.json';
import noMessages from '../languages/no/experience.json';

type Kind = 'work' | 'edu';
type Item = {
  type: Kind; title: string; company: string; location?: string; date: string;
  highlights: string[]; tech: string[];
};
type Messages = { title: string; subtitle: string; work: string; education: string; items: Item[]; };

function useLocaleMessages(): Messages {
  const pathname = usePathname();
  const locale = pathname?.split('/')[1] === 'no' ? 'no' : 'en';
  return (locale === 'no' ? noMessages : enMessages) as Messages;
}
    ...
    ">
      ...
      {shownHighlights.length > 0 && (
        <ul className="mt-3 list-disc space-y-2 pl-5 text-sm text-zinc-700 dark:text-zinc-300">
          {shownHighlights.map((h, i) => <li key={i}>{h}</li>)}
        </ul>
      )}

      {(item.tech.length > 0 || hasMore) && (
        <div className="mt-3 flex flex-wrap items-center gap-2">
          {shownTech.map((t, i) => <Pill key={i}>{t}</Pill>)}
...
export default function ExperienceSection() {
  ...
  const items = useMemo(() => t.items.filter(i => i.type === tab), [t.items, tab]);
  ...
  return (
    <section id="experience" className="mx-auto my-16 w-full max-w-3xl px-4">
      ...
      <ol className="relative pl-6 border-l border-zinc-200 dark:border-zinc-800/70">
        {items.map((item, idx) => (
